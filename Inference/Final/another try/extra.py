...
def convert_to_horn_clause(cnf_sentence):
    ...
    for clause in clauses:
        ...
        # print(f"this is after disjuncts: {literals}")
        ...
        if isinstance(clause, Expr) and clause.op == '|':
            ...
            # print(f"pos lit: {positive_literals}\nneg lit: {negative_literals}")
        ...
        horn_clause = convert_expr_to_str(horn_clause)
        horn_clauses.append(horn_clause)

    # Combine the Horn clauses into a single sentence
    # horn_sentence = associate('&', horn_clauses)
    return horn_clauses


def convert_to_horn_form(formula):
    cnf_sentence = to_cnf(formula)
    # print(f"this is after cnf: {cnf_sentence}")
